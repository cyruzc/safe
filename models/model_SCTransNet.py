...
try:
    ...
    def rearrange(tensor, pattern, **axes_lengths):
        ...
        if pattern == 'b (h w) c -> b c h w':
            h = axes_lengths['h']
            w = axes_lengths['w']
            return tensor.transpose(1, 2).reshape(tensor.shape[0], -1, h, w)
        if pattern == 'b c (h w) -> b c h w':
            h = axes_lengths['h']
            w = axes_lengths['w']
            return tensor.reshape(tensor.shape[0], tensor.shape[1], h, w)
        if pattern == 'b (head c) h w -> b head c (h w)':
            b, head_c, h, w = tensor.shape
            head = axes_lengths['head']
            c = head_c // head
            return tensor.reshape(b, head, c, h * w)
        raise NotImplementedError(f"Fallback rearrange does not support pattern: {pattern}")
import numbers
try:
    from thop import profile
except ImportError:
    def profile(*args, **kwargs):
        raise ImportError("thop is only required for FLOPs profiling, not inference.")
