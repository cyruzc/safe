...
try:
    ...
    from .DCN.modules.deform_conv import DeformConvPack as DCN
except ImportError:
    try:
        # Try from models.DCNv2.DCN
        from models.DCNv2.DCN.modules.deform_conv import DeformConvPack as DCN
    except ImportError:
        # Fallback: try to import DCN directly (if compiled)
        try:
            from DCN.modules.deform_conv import DeformConvPack as DCN
        except ImportError:
            # If all fails, create a dummy placeholder (will fail at runtime)
            import warnings
            warnings.warn("DCN not available, TTOA will not work properly")
            DCN = None
